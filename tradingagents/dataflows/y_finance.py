...
import os
from .stockstats_utils import StockstatsUtils, _clean_dataframe, yf_retry, yf_call_with_crumb_retry, load_ohlcv, filter_financials_by_date
...
def get_YFin_data_online(
    ...
    data = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).history(start=start_date, end=end_date))
...
def get_fundamentals(
    ...
    try:
        info = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).info)
...
def get_balance_sheet(
    ...
    try:
        if freq.lower() == "quarterly":
            data = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).quarterly_balance_sheet)
        else:
            data = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).balance_sheet)
...
def get_cashflow(
    ...
    try:
        if freq.lower() == "quarterly":
            data = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).quarterly_cashflow)
        else:
            data = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).cashflow)
...
def get_income_statement(
    ...
    try:
        if freq.lower() == "quarterly":
            data = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).quarterly_income_stmt)
        else:
            data = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).income_stmt)
...
def get_insider_transactions(
    ...
    canonical = normalize_symbol(ticker)
    try:
        data = yf_call_with_crumb_retry(lambda: yf.Ticker(canonical).insider_transactions)
